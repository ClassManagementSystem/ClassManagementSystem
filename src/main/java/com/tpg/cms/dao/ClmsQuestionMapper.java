...
public interface ClmsQuestionMapper {
    ...
    // 获 取 问 题 的 答 复 数 量 ，通 过 id 确 定
    @Select("select count(*) from clms_question where question_id = #{qid} and question_status = 0 ")
    ...
    @Select("<script>" +
            ...
            "            and question_content like CONCAT('%', #{params.question_content}, '%')\n" +
            "        </if>\n" +
    ...
    //分 页 查 询
    @Select("<script>" +
            "        select question_id, question_title, question_content, question_author, question_time, update_time, question_good, answer_count, question_mark from clms_question\n" +
            "        where is_deleted = 0 \n" +
            "        <if test=\"params.question_title!=null and params.question_title!=''\">\n" +
            "            and question_title like CONCAT('%', #{params.question_title}, '%')\n" +
            "        </if>\n" +

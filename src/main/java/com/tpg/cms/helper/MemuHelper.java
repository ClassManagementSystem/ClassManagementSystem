...
public class MemuHelper {
     ...
     * @return
     */
    public static List<JSONObject> bulid(List<SysPermission> treeNodes) {
        List<JSONObject> meuns = new ArrayList<>();
        if(treeNodes.size() == 1) {
            SysPermission topNode = treeNodes.get(0);
            //左侧一级菜单
            List<SysPermission> oneMeunList = topNode.getChildren();
            for(SysPermission one :oneMeunList) {
                ...
                List<SysPermission> twoMeunList = one.getChildren();
                for(SysPermission two :twoMeunList) {
